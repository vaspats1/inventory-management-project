...
class Game {
  ...
  init() {
    ...
    // Disable control zones while menu is visible so they don't intercept touches
    document.getElementById('controls-layer').style.display = 'none';
    document.getElementById('cam-zone').style.pointerEvents = 'none';

    // Menu team select — use touchstart for reliable iOS response
    const pick = (team) => this.startGame(team);
    document.getElementById('btn-paok').addEventListener('touchstart', e => { e.preventDefault(); e.stopPropagation(); pick('PAOK'); }, { passive: false });
    document.getElementById('btn-iraklis').addEventListener('touchstart', e => { e.preventDefault(); e.stopPropagation(); pick('IRAKLIS'); }, { passive: false });
    document.getElementById('btn-paok').addEventListener('click', () => pick('PAOK'));
    document.getElementById('btn-iraklis').addEventListener('click', () => pick('IRAKLIS'));
  }
  ...
  startGame(teamKey) {
    ...
    document.getElementById('controls-layer').style.display = '';
    document.getElementById('cam-zone').style.pointerEvents = '';
